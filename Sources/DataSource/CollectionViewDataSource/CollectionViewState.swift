...
public enum CollectionViewState: Equatable, Sendable {
    case initial
    case loading(previousSections: [CollectionSection]? = nil)
    case loadingMore(currentSections: [CollectionSection], pagination: PaginationInfo?)
    case refreshing(currentSections: [CollectionSection])
    case loaded(sections: [CollectionSection], pagination: PaginationInfo?)
    case empty(config: EmptyStateConfiguration)
    case error(ViewError, previousSections: [CollectionSection]? = nil)
    case loadingMoreError(ViewError, currentSections: [CollectionSection], pagination: PaginationInfo?)

    public static func == (lhs: CollectionViewState, rhs: CollectionViewState) -> Bool {
        switch (lhs, rhs) {
